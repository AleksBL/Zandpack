...
HubDer  = None#{'C':'-0.1492',
              #'H':'-0.1857'}
DampCor =None#'4.00'
slako = '/home/investigator/Desktop/slako/matsci/matsci-0-3/'# /home/aleksander/Desktop/slako/3ob/3ob-3-1/'

g    = sisl.geom.nanoribbon(width=7, bond=1.42, atoms = 6).add_vacuum(10,1).add_vacuum(50,2)
gem  = g.tile(2,0)
gep  = g.tile(2,0).move(2 * g.cell[0]*(txd-1))
gd   = g.tile(2 * txd, 0)#.remove([44,47,54,57])
CR   = gd.center()
g_al = g_al.move(CR)
geo  = geo.move(CR)
gd   = gd.add(g_al)
...
EO  = SiP(geo.cell, geo.xyz, geo.atoms.Z, 
          directory_name = 'EO',
          sl = 'EO',
          dm_tol   ='1e-7',
          pp_path  = slako,
...
# EM.Passivate(Hatom, 1.05, cond_filling=True,condition=cond)
# EP.Passivate(Hatom, 1.05, cond_filling=True,condition=cond)
...
EO.write_and_run_dftb_in_dir(angmom,)
#assert 1 == 0
...
bzo  = sisl.MonkhorstPack(hep, [1,1,50])
...
muo  = heo.fermi_level(bz = bzp, q = EO.ideal_cell_charge( {'Al':3.0        } ))
...
bando = sisl.BandStructure(heo, [[.0, .0, .0], [.0, .0, .5]],100)
em,ep,eo = np.zeros((100,hem.no)), np.zeros((100, hep.no)), np.zeros((100, heo.no))
for i in range(100):
    em[i] = hem.eigh(k=bandm.k[i])
    ep[i] = hep.eigh(k=bandp.k[i])
    eo[i] = heo.eigh(k=bando.k[i])

#assert 1 == 0

#idx = np.arange(len(gd.xyz))
#np.random.shuffle(idx)

Dev = SiP(gd.cell, gd.xyz, gd.atoms.Z, 
          directory_name = 'Dev',dm_tol='1e-7',
          pp_path  = slako,
...
# Dev.Passivate_with_molecule(Hatom, 1.05, cond_filling=True,cond=cond)
...
mud = Hd.fermi_level(bz = bzd, 
                     q  = Dev.ideal_cell_charge({'Al':3.0,'C':4.0, 'H':1.0})
                     )
...
for _k in Kv:
    ...
    #    vec = v[:,i]
    #    dm_d += vec[:, None] * vec[None,:].conj() * FD[i]/5
