...
M1 = (
    e1.tile(tx1, 0)
    .move([vx1, vy1, 0])
    .sub([i for i in range(e1.na * tx1) if i not in r_idx_1])
)
M2 = (
    e2.tile(tx2, 0)
    .move([vx2, vy2, 0])
    .sub([i for i in range(e1.na * tx1) if i not in r_idx_2])
)
...
dev = dev.move(
    dev.center(what="cell") - dev.center(),
)
...
E1.write_and_run_dftb_in_dir(
    angmom,
)
E2.write_and_run_dftb_in_dir(
    angmom,
)
nk = 100
hem = E1.dftb2sisl()
hep = E2.dftb2sisl()
bp = sisl.BandStructure(hem, [[0.0, 0.0, 0.0], [0.5, 0.0, 0.0]], nk)
eigm = np.zeros((nk, hem.no))
eigp = np.zeros((nk, hep.no))
for i in range(nk):
    eigm[i] = hem.eigh(k=bp.k[i])
    eigp[i] = hep.eigh(k=bp.k[i])
hem.set_nsc((3, 1, 1))
hep.set_nsc((3, 1, 1))
eigm2 = np.zeros((nk, hem.no))
eigp2 = np.zeros((nk, hep.no))
for i in range(nk):
    eigm2[i] = hem.eigh(k=bp.k[i])
    eigp2[i] = hep.eigh(k=bp.k[i])
hem.set_nsc((3, 1, 1))
hep.set_nsc((3, 1, 1))
...
D.Passivate_with_molecule(H, 1.1,  cond_filling=True, cond=PCond)
E1.Passivate_with_molecule(H, 1.1, cond_filling=True, cond=PCond)
E2.Passivate_with_molecule(H, 1.1, cond_filling=True, cond=PCond)
D.find_elec_inds()

def Buffer(r):
    if (
        r[0] < E1.pos_real_space[:, 0].min() - 0.1
        or r[0] > E2.pos_real_space[:, 0].max() + 0.1
    ):
        return True
    else:
        return False
...
D.write_and_run_dftb_in_dir(
    angmom,
)
Hd = D.dftb2sisl()
Hd.set_nsc((1, 1, 1))
mud = Hd.fermi_level(
    bz=sisl.MonkhorstPack(Hd, [1, 1, 1]),
    q=E2.ideal_cell_charge({"C": 4.0, "H": 1.0}),
)
Hd.shift(-mum -0.15)
D.manual_H(Hd)
E1.manual_H(hem)
