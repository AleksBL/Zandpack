...
# In[]
Test = load(open('Test.Timedep','rb'))
tbt = sisl.get_sile(Test.Device.dir + '/siesta.TBT.nc')


# In[]
Eg = np.linspace(-3,3,300)
#Test.reset_all_fits()

NL = 41
init_E = [np.linspace(-2.5,2.5,NL)[None,:]]*2
init_G = [np.array([3.5/NL]*NL)[None,:]]*2
alpha_PO = 0.0001
...
# v1 = Test.fitted_lorentzians[0].is_positive_semidefinite(Eg, Test._Slices)
# v2 = Test.fitted_lorentzians[1].is_positive_semidefinite(Eg, Test._Slices)

# # In[]
# EI = [Test.NO_fitted_lorentzians[0].ei, Test.NO_fitted_lorentzians[1].ei]
# WI = [Test.NO_fitted_lorentzians[0].gamma, Test.NO_fitted_lorentzians[1].gamma]
