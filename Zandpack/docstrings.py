...
scf_name = """
  ▂▃▅▆▇█ S █ C █ F █(v. """+str(__version__)+""")█▇▆▅▃▂
"""+logo

psi0_name = """
  ▂▃▅▆▇█  psinought (v. """+str(__version__)+""")█▇▆▅▃▂
""" +logo

MPI_ProgramNameNO ="""
  ▂▃▅▆▇█   nozand   (v. """+str(__version__)+""")█▇▆▅▃▂
"""+logo
...
DocString_MPI_implementationNO = \
  ...
  Program developed for time-dependent transport in nanostructures at the Technical University of Denmark (DTU).

  """ + CiteString +"""

  The program is distributed under the MPLv2.0 licence.

  Author: Aleksander Bach Lorentzen ( aleksander.bl@proton.me )
  ...
  It is recommended to use the Zandpack wrapper functions for carrying out calculations.
  ...
  Brief outline of workflow:
      ...
      3. Do the calculation using the steady-state as initial state.
...
scf_docstring = \
    ...
    An orthogonal basis is assumed.
